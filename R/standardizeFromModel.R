...
#'
#' Uses the provided list of null models (${\rm modelL}$) to standardize each type of
#' correlations to the target common null model specified by the v.std parameter,
#' $(r + 1)/2 ~ {\rm Beta}({\rm v.std}, {\rm v.std})$, where $r$ is the
...
#' in ${\rm modelL}$. The resulting standardized correlation matrix is built blockwise and is full symmetric.
#'
#' @param modelL A list-of-lists structure providing two inputs for each pair of 'Omics datasets $A$ and $B$: a matrix ${\rm modelL}\[\[A\]\]\[\[B\]\]\[\['cor'\]\]$ of raw Spearman correlation values, and shape parameters ${\rm modelL}\[\[A\]\]\[\[B\]\]\[\['shape'\]\] == {\rm c}\(v,w\)$ specifying a null model $(1+r_{raw})/2 ~ {\rm Beta}(v,w)$ for the raw correlations.
#' @param analtyeL For each 'Omics dataset $A$, ${\rm analyteL}\[\[A\]\]$ lists the analytes provided by dataset $A$. These analyte identifiers are required to be unique across all datasets.
#' @param v.std 
