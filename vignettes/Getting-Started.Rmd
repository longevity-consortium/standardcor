...
beta.shape <- estimateShape(Z[row(Z) < col(Z)], plot=TRUE, ylim=c(0,14), fine=100,
                            main=paste(Samples,"samples,",Dimensions,"variables"))
...
For the second example we modify the test data, mixing a Bias variable (copied the required number of times) with the first $20\%$ of the variables. The first 20% of the variables are
therefore intercorrelated, in this case with a correlation coefficient of $r = 0.5$ (in addition to whatever spurious correlation they had to start with). Pairwise correlations among these
variables are not purely spurious correlations, and they affect $20\% \times 20\% = 4\%$ of the unique, non-identical pairs; the remaining 96% of the pairs represent purely spurious
...
# Adjusted Step 1
M2 <- M
Altered <- round(Dimensions*0.2) # The first Altered variables will be intercorrelated
Bias <- matrix(rep(rnorm(Samples),Altered),nrow=Samples,ncol=Altered)

r <- 0.5  # the amount of correlation
M2[,c(1:Altered)] <- (1-r) * M2[,c(1:Altered)] + r*Bias
Z2 <- SparseSpearmanCor2(M2)

# Repeat Step 2
beta.shape <- estimateShape(Z2[row(Z2) < col(Z2)], plot=TRUE, ylim=c(0,14), fine=100,
                            main=paste(Samples,"samples,",Dimensions,"variables,",Altered,"intercorrelated with r =",r))
title(paste(round(beta.shape,2),collapse=", "),line=-1)
#text(x=-0.5,y=2.0,labels=paste("D/2 - 1 = ",round(Dimensions/2-1, 2)),adj = 1/2)
